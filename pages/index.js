...
function Sel({ label, value, onChange, options }) {
  return (
    <div style={{ background:'#161922', borderRadius:4 }}>
      <label style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:9, letterSpacing:'1.5px', textTransform:'uppercase', color:'#6b7a96', fontWeight:700, marginBottom:4, display:'block', paddingTop:6, paddingLeft:11 }}>{label}</label>
      <select value={value} onChange={e=>onChange(e.target.value)} style={{ width:'100%', background:'#161922', border:'1px solid #1e2330', borderRadius:4, padding:'7px 11px 9px', color:'#f2f4f8', fontFamily:'inherit', fontSize:13, outline:'none', appearance:'none', WebkitAppearance:'none', colorScheme:'dark', forcedColorAdjust:'none', display:'block' }}>
        {options.map(o => <option key={o} style={{ background:'#161922', color:'#f2f4f8' }}>{o}</option>)}
      </select>
    </div>
  )
}
                ...
                {sportTeams.map(team => {
                  ...
                  return (
                    <div key={team.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:current?.id===team.id?al(P,0.1):'#161922', border:`1px solid ${current?.id===team.id?al(P,0.4):'#1e2330'}`, borderRadius:6, marginBottom:6 }}>
                      ...
                      {onOpenTeamTab && <button onClick={(e)=>{ e.stopPropagation(); switchTeam(team); onOpenTeamTab() }} style={{ padding:'3px 7px', background:al(P,0.12), border:`1px solid ${al(P,0.3)}`, borderRadius:3, color:P, fontSize:9, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700 }}>OPEN →</button>}
